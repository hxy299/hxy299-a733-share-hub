...
int main()
{
  ...
  assert(aipet::tts_text_frame("Hello", frame));
  assert((frame == std::vector<std::uint8_t>{0xfd,0,7,1,0,'H','e','l','l','o'}));
  assert(aipet::tts_text_frame(std::string("\xc4\xe3\xba\xc3",4), frame));
  assert(frame.size() == 9 && frame[2] == 6);
  assert(!aipet::tts_text_frame("你好", frame));
  assert(!aipet::tts_text_frame(std::string(4095,'a'), frame));
  assert(aipet::tts_text_frame(std::string(4094,'a'), frame));
  ...
  assert(!port.speak_gb2312("Hello",5,5,5));
