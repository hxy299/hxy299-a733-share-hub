...
1. 真机验证 v67 WAPI 的 2.4/5 GHz 扫描、WPA2、DHCP、重连和断开；
2. 验证通过后，将 `/dev/a733-wifi` 写控制接口标记为兼容模式，默认只读诊断；
3. 为连接状态、RSSI、统计补齐标准 WEXT 查询，减少 `wifi` 对私有诊断文本的依赖；
4. USB 摄像头完成后注册标准 `/dev/video0` 并使用 openvela video API；
