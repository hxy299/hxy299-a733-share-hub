...
串口/SSH 文本
  ...
  -> 回复标签解析
  -> 终端文字输出
  -> 可选 UART4 TW-TTS（已构建，待实机发声验证）
```

其中，联网初始化、API Key 加密保存、Agent 自启动、DeepSeek 连续对话已经通过实机测试。USB/I2S 麦克风、I2S 扬声器、动作执行、屏幕表情和摄像头到 YOLO 的完整产品链路尚未打通。本地 Qwen2.5-1.5B 已有独立可运行引擎，但还没有注册到桌宠路由器，因此当前不能把它称为桌宠的在线失败回退。
