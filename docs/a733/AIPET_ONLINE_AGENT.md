...
- 将官方 Agent 和桥接源加入实际构建/服务启动，目前尚未注册可运行桌宠命令。
- 关闭非必要工具，特别是任意 shell/文件修改，保留桌宠受控动作；先审查官方 tool_guard 策略。
- 云端 CA 和主机名严格验证：当前官方 TLS 的 VERIFY_OPTIONAL 必须审计，不能通过关闭验证“修复”网络。
